...
//        $errors = $smsMgr->sendSmsVerifyCode($smsVerify->getMobile(), $smsVerify->getCode(), $smsVerify->getExpiryDuration());
    ...
    public function getTokenUserById($values){
       $output = array('status' => 'no','errorCode' => 0,'errorMsg' =>'' ,'results' => array()); // default status is false.
       if (isset($values['id']) == false || isset($values['username']) == false) {
           $output['errorCode'] = 400;
           $output['errorMsg'] = 'Wrong parameters.';
           return $output;
       }
       $tokenUser = $this->createWapTokenUser($values['id'],$values['username'],$values['userHostIp']);
       if (isset($tokenUser)) {
            $output['errorCode'] = 0;
            $output['errorMsg'] = 'success';
            $output['status'] = 'ok';
            $output['results']['token'] = $tokenUser->getToken();
            $output['results']['user_id'] = $tokenUser->getUserId();
            $output['results']['username'] = $tokenUser->getUsername();
            // TODO: log.
        } else {
            $output['errorCode'] = ErrorList::ERROR_TOKEN_CREATE_FAILED;
            $output['errorMsg'] = '生成token失败!';
