 ...
 * @property integer $is_contracted
 * @property integer $hospital_id
 * @property string $hospital_name
 * @property integer $hp_dept_id
 * @property string $hp_dept_name
 * @property string $faculty
 * @property integer $medical_title
 * @property integer $academic_title
 * @property integer $expteam_id
 * @property integer $state_id
 * @property integer $city_id
 * @property integer $gender
 * @property string $career_exp
 * @property string $description
 * @property string $honour
 * @property string $reason_one
 * @property string $reason_two
 * @property string $reason_three
 * @property string $reason_four
 ...
 * @property User $user
 * @property DoctorCert[] $doctorCerts
 * @property ExpertTeam[] $expertTeams
 * @property HospitalDeptDoctorJoin[] $hospitalDeptDoctorJoins
 */
class Doctor extends EActiveRecord
{
    ...
    {
        return [
            [['user_id', 'is_contracted', 'hospital_id', 'hp_dept_id', 'medical_title', 'academic_title', 'expteam_id', 'state_id', 'city_id', 'gender', 'has_remote', 'role', 'sub_cat_id', 'display_order'], 'integer'],
            ...
            [['name', 'fullname', 'hospital_name', 'hp_dept_name', 'faculty', 'password_raw', 'wechat', 'tel'], 'string', 'max' => 50],
            [['mobile'], 'string', 'max' => 11],
            [['career_exp'], 'string', 'max' => 2000],
            [['description'], 'string', 'max' => 500],
            [['honour'], 'string', 'max' => 1500],
            [['reason_one', 'reason_two', 'reason_three', 'reason_four'], 'string', 'max' => 30],
            [['specialty', 'disease_specialty', 'surgery_specialty', 'avatar_url', 'remote_file_key'], 'string', 'max' => 200],
            [['search_keywords', 'email', 'sub_cat_name'], 'string', 'max' => 100],
            [['base_url', 'remote_domain'], 'string', 'max' => 255],
    ...
    public function attributeLabels()
    {
        return [
         ...
         /*   'reason_one' => 'Reason One',
          ...
          /*  'specialty' => 'Specialty',
            'disease_specialty' => 'Disease Specialty',
            'surgery_specialty' => 'Surgery Specialty',
    ...
    {
        return $this->hasMany(Booking::className(), ['doctor_id' => 'id']);
    }
     ...
     * @return \yii\db\ActiveQuery
     */
    public function getDoctorCert()
    {
        return $this->hasMany(DoctorCert::className(), ['doctor_id' => 'id']);
    }
    ...
    {
        return $this->hasMany(ExpertTeam::className(), ['leader_id' => 'id']);
    }

    /**
     * @return \yii\db\ActiveQuery
     */
    public function getHospitalDeptDoctorJoins()
    {
        return $this->hasMany(HospitalDeptDoctorJoin::className(), ['doctor_id' => 'id']);
    }
    ...
    public function getAllByHpDeptId($deptId, $with = null, $options = null) {
       /* $criteria = new CDbCriteria;
        ...
        $criteria =new ActiveQuery('app\models\doctor\Doctor');
        $criteria->select('t.*');
        $criteria->distinct= FALSE;
        if (isset($with)) {
            $criteria->with($with);
        ...
        $criteria->join='left join hospital_dept_doctor_join j on (t.`id`=j.`doctor_id`)';
        $criteria->andWhere("j.hp_dept_id=".$deptId);
        $criteria->orderBy= "j.`display_order` ASC";
        if (isset($options['offset'])) {
            $criteria->offset($options['offset']);
            $criteria->limit(10);  // limit must be defined if offset is applied.
        }
