...
use Yii;
use app\models\base\ESearchModel;
class DoctorSearch extends ESearchModel {
    public function __construct($searchInputs, $with = null,$classname=null) {
        $searchInputs['order'] = 't.is_contracted DESC, t.role, t.medical_title, convert(t.name using gbk)';
        parent::__construct($searchInputs, $with,$classname);
    }
    ...
    public function addQueryConditions() {
        $this->criteria->andOnCondition('t.date_deleted is NULL');

        if ($this->hasQueryParams()) {
            // Doctor.medical_title
            if (isset($this->queryParams['mtitle'])) {
                $mtitle = $this->queryParams['mtitle'];
                $this->criteria->andWhere('t.medical_title', $mtitle);
            }
