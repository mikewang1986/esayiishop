    ...
    private function setBookings($models) {
        if (arrayNotEmpty($models)) {
            foreach ($models as $model) {
                ...
                //加入判断
                if(in_array($data->bkStatus,$this->sortwap)){
                    $this->sortarray[$data->bkStatus][]=$data;
                }
                else{
        ...
        if (arrayNotEmpty($this->sortarray)) {
              ksort($this->sortarray);
              foreach($this->sortarray as $key1=>$value1){
                  $this->results->booking[]=$value1;
              }
        }
        }else {
